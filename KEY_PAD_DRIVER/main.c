...
{
	LCD_set_cursor(0,1);
	LCD_sendstring(the_message, 0);
	LCD_set_cursor(0,1);
	_delay_ms(200);
	LCD_sendstring("               ", 0);
	LCD_set_cursor(current_x,0);
}

void display_ans(s32 ans)
{
	LCD_set_cursor(0,1);
	LCD_sendstring("        ", 0); //to clear the second row
	LCD_set_cursor(0,1);
	LCD_sendstring("= ", 0);

	LCD_send_number((u32)ans);
}
	...
	while(1)
	{
		...
		if((key_press != 0))
		{
			if(key_press == 12) //equals sign
			{
				...
				LCD_sendChar(char_key_map[key_press]);
